...
//#define MAX 68.0
    ...
    //****************************************************************************
    // Compile
    // $ gcc -Wall formiga_app3.c -o formiga -lm
    // $ ./formiga
    //
    // Instruções:
...
void criaItens(Item **it, Config *cf){
    char url[]="breast-cancer-wisconsin.data";
    int ni = 0, exc = 0, i;
    char linha[50] = {'\0'};
    int v[9];
    char c;
    ...
    *it = (Item*)malloc(699*sizeof(Item));
    ...
    if(arq == NULL)
        printf("Erro, nao foi possivel abrir o arquivo\n");
			...
			while( (fscanf(arq,"%d,%s\n", &id, linha)!=EOF)){
                ...
                if(linha[i]=='\0'){
                    sscanf(linha, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%c", &v[0], &v[1], &v[2], &v[3], &v[4], &v[5], &v[6], &v[7], &v[8], &c);
                    //printf("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%c\n", id, v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], c);
                }
                for(i=0;i<9;i++){
                    (*it)[ni].v[i] = v[i];
                }
...
double distancia(Item i1, Item i2){
	...
	for(i=0;i<9;i++){
		soma += pow(((double)(i1.v[i]) - (double)(i2.v[i])),2);
	}
	dist = sqrt(soma);
	return dist;
}
