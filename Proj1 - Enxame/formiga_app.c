...
#define MAX 68.0
    ...
    //****************************************************************
    // Compile
    // $ gcc -Wall formiga_app.c -o formiga -lm
    // $ ./formiga
    //
    // Instruções:
...
void criaItens(Item **it, Config *cf){
    char url[]="base.txt";
	char stringa[50], stringb[50], sn1[20], sn2[20];
    double n1, n2;
	int i = 5, iA;
    int virg;
    int ni = 0;
    ...
    *it = (Item*)malloc(400*sizeof(Item));
    cf->itens = 400;
    ...
    if(arq == NULL)
        printf("Erro, nao foi possivel abrir o arquivo\n");
			...
			for (i=0; i<4; i++){
				fscanf(arq, "#%s %s -> %i\n",stringa, stringb, &iA);
				//printf("i:%i %s %s -> %i\n",i, stringa, stringb, iA);
			}
			fscanf(arq, "\n");
			
			while( (fscanf(arq,"%s%c%s%c%c\n", sn1, &tab, sn2, &tab, &c)!=EOF)){
                virg = 0;
                i = 0;
                while(virg<2){
                    if(sn1[i] == ','){
