...
def ast(matriz, inicio, fim, lista, const):
    i0 = inicio[0]
    i1 = inicio[1]
    f0 = fim[0]
    f1 = fim[1]
    ...
    lista.append([i0, i1, (const*distancia(inicio, fim))])
    matriz[i0][i1][1] = 0
    matriz[i0][i1][2] = const*distancia(inicio, fim)
    while(encontrou == 0):
        if(len(lista) == 0):
        ...
        if(total != (matriz[a0][a1][1] + matriz[a0][a1][2])):
            continue
        ...
        if(a0 > 0 and (matriz[a0-1][a1][1] == -1 or (matriz[a0][a1][1] + peso(matriz[a0-1][a1][0]) < matriz[a0-1][a1][1]) ) ):
            custo = matriz[a0][a1][1] + peso(matriz[a0-1][a1][0])
            total = custo + const*distancia([a0-1, a1], fim)
            pos = posInsert(lista, total)
            if(pos==-1):
                break
            lista.insert(pos, [a0-1, a1, total])
            matriz[a0-1][a1][1] = custo
            matriz[a0-1][a1][2] = const*distancia([a0-1, a1], fim)
            nos+=1
        #leste
        if(a1 < 41 and (matriz[a0][a1+1][1] == -1 or (matriz[a0][a1][1] + peso(matriz[a0][a1+1][0]) < matriz[a0][a1+1][1]) )):
            custo = matriz[a0][a1][1] + peso(matriz[a0][a1+1][0])
            total = custo + const*distancia([a0, a1+1], fim)
            pos = posInsert(lista, total)
            if(pos==-1):
                break
            lista.insert(pos, [a0, a1+1, total])
            matriz[a0][a1+1][1] = custo
            matriz[a0][a1+1][2] = const*distancia([a0, a1+1], fim)
            nos+=1
        #sul
...
def exec_mouse(matriz, const, lista):
    inicio = []
    fim = []
    posicoes(matriz, inicio, fim)
    ast(matriz, inicio, fim, lista, const)

def exec_teste(matriz, const, lista):
    ...
    for i in range(0, 10):
        print(i)
        aux = deepcopy(matriz)
        (p, n) = ast(aux, inicio[i], fim[i], lista, const)
        pesos.append(p)
        nos.append(n)
...
#ast(matriz, inicio, fim, lista, const)
#exec_mouse(matriz, const, lista)
exec_teste(matriz, const, lista)
